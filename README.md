...
Wynntils Overlay Editor 是一个非官方社区工具，用于创建和预览 Wynntils Overlay，当前重点支持 Info Box。它是不需要构建步骤的静态网页：普通用户打开托管地址即可使用，无需安装 Node.js，也无需连接其他宿主项目。

本工具不会导出完整的 Wynntils JSON，也不会替用户创建游戏内 overlay。预览使用固定示例状态，主要用于检查 Content 的布局、颜色、字体和格式码；最终效果以 Wynntils 游戏客户端为准。

## 快速使用

1. 打开项目提供的托管网页。
2. 选择一个示例，或在 Content 字段中输入自己的模板。
3. 使用函数目录搜索并插入表达式，查看右侧预览和校验提示。
...
- 直接在浏览器中编辑 Wynntils Content，保留输入法、选择、撤销和键盘操作。
- 离线函数搜索、语法高亮、校验、示例说明和 Canvas 预览。
- 对无法依赖游戏实时状态的函数显示明确的示例值或占位提示。
- 可选的 OpenAI-compatible AI 助手；不会自动覆盖 Content，应用前需要用户确认。
- 草稿仅保存显式允许的 Info Box 配置，不保存 API Key、端点、模型或 AI 对话。

## 许可证

工具自身代码使用 MIT 许可证，见 [LICENSE](LICENSE)。仓库中的 Wynntils 函数元数据、翻译和字体位图属于第三方内容，许可证和来源见 [LICENSES/README.md](LICENSES/README.md) 与 [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md)。
